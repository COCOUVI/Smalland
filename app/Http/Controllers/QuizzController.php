   ...
   public function submit(Request $request, $quizzId)
{
    ...
    if ($userQuizz && $userQuizz->updated_at) {
        if ($userQuizz->score < 70 && now()->diffInHours($userQuizz->updated_at) < 24) {
    ...
    $allPerfect = $userQuizzScores->count() === $quizzIds->count() && $userQuizzScores->every(fn($score) => $score >= 70);
